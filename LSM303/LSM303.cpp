...
uint8_t LSM303::readAccReg(uint8_t reg)
{
  ...
  _i2c->read(acc_address,reading,1);

  return (uint8_t)reading[0];
}
...
uint8_t LSM303::readMagReg(int reg)
{
  ...
  _i2c->read(mag_address,reading,1);
...
uint8_t LSM303::readReg(int reg)
{
  // mag address == acc_address for LSM303D, so it doesn't really matter which one we use.
...
void LSM303::readMag(void)
{
  ...
  int16_t readingInt[3];
  char* reading = (char*)readingInt; //treat the int16_t array as a 6 element char array
