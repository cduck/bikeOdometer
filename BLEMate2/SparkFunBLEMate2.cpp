...
BLEMate2::opResult BLEMate2::addressQuery(String &address)
{
  ...
  _serialPort->puts("VER\r");
  //_serialPort->flush();  // Flushing is automatic
...
BLEMate2::opResult BLEMate2::stdCmd(String command)
{
  ...
  _serialPort->puts(command.c_str());
  _serialPort->puts("\r");
  //_serialPort->flush();
  ...
  {
    // Grow the current buffered data, until we receive the EOL string.    
    if (_serialPort->readable() > 0) buffer.concat(char(_serialPort->getc()));
...
BLEMate2::opResult BLEMate2::stdSetParam(String command, String param)
{
  ...
  _serialPort->puts("SET ");
  _serialPort->puts(command.c_str());
  _serialPort->puts("=");
  _serialPort->puts(param.c_str());
  _serialPort->puts("\r");
  //_serialPort->flush();
    ...
    if (buffer.endsWith(EOL))
    {
      ...
      if (buffer.startsWith("RE")) 
      {
        stdCmd("SCN OFF"); // When we come out of reset, we *could* be
                           ...
                           //  random and noisy.
        wait_ms(500);        // Let the scanning noise complete.
...
BLEMate2::opResult BLEMate2::amCentral(bool &inCentralMode)
{
  String buffer;
  String EOL = String("\n\r");
